...
}:
{
  ...
  options = {
    my.kde.enable = lib.mkEnableOption "enables KDE config";
  };
  ...
  config = lib.mkIf config.my.kde.enable {
    ...
    programs.plasma = {
      enable = true;
      # Stylix writes to KDE files
      overrideConfig = true;

      # https://nix-community.github.io/plasma-manager/options.xhtml#opt-programs.plasma.configFile
      configFile = {
        "kglobalshortcutsrc" = {
          # [ActivityManager]
          # _k_friendly_name=Activity Manager
          # switch-to-activity-8dfe5c8a-9c15-4ff3-a5ea-239cdaa56380=none,none,Switch to activity "Default"
          ...
          "plasmashell" = {
            ...
            "activate task manager entry 6" = "none,,Activate Task Manager Entry 6";
            "activate task manager entry 7" = "none,,Activate Task Manager Entry 7";
            "activate task manager entry 8" = "none,,Activate Task Manager Entry 8";
            "activate task manager entry 9" = "none,,Activate Task Manager Entry 9";
            "activate task manager entry 10" = "none,,Activate Task Manager Entry 10";
      ...
      # Use Meta + number to jump to workspace
      # Use Meta + Q to quit (kill) applications
      # Use Meta + SPC to open Krunner
      # M+t -> kitty
      hotkeys.commands = {
        "launch-kitty" = {
          name = "Launch Kitty";
          ...
          # logs.enabled = false; # if you don't want systemd-cat wrapping
        };
      };
